...
export default function MonthlyPicks({ isAdmin, artists, onNavigateToArtist }) {
  ...
  const [scores, setScores] = useState({}) // pickId -> { adminScore, userScore }
  ...
  const loadPicks = async () => {
    ...
    // Load scores for each pick that has an artist + matching song/album
    const scoreMap = {}
    for (const pick of pickList) {
      if (!pick.artist_id) continue

      if (pick.type === 'single') {
        // Find song by title across all albums
        const { data: songs } = await supabase
          .from('songs')
          .select('id, albums(artist_id)')
          .eq('name', pick.title)
        ...
        if (songIds.length) {
          ...
          scoreMap[pick.id] = { adminScore: adminRating, allRatings }
        }
      } else {
        // Album — get all songs for matching album name
  ...
  const ScoreDisplay = ({ pickId }) => {
    ...
    return (
      <div className="text-right">
        ...
        {s.adminScore === null && <span className="text-zinc-600 text-sm">—</span>}
      </div>
    )
  }

  const PickTable = ({ title, items, type, onAdd }) => (
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select value={formArtistId} onChange={e => { setFormArtistId(e.target.value); setFormArtistName('') }}
            ...
            {!formArtistId && (
              <input value={formArtistName} onChange={e => setFormArtistName(e.target.value)}
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => addPick(type)} disabled={saving}
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddSingle && <AddModal type="single" onClose={() => setShowAddSingle(false)} />}
      {showAddAlbum && <AddModal type="album" onClose={() => setShowAddAlbum(false)} />}
    </div>
  )
}
